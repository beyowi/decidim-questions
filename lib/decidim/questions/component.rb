...
Decidim.register_component(:questions) do |component|
  ...
  component.newsletter_participant_entities = ["Decidim::Questions::Question"]
  ...
  component.settings(:global) do |settings|
    ...
    settings.attribute :participatory_texts_enabled,
                       ...
                       readonly: ->(context) { Decidim::Questions::Question.where(component: context[:component]).any? }
    ...
    settings.attribute :new_question_body_template, type: :text, translated: true, editor: false, required: false
  ...
  component.register_stat :questions_accepted, primary: true, priority: Decidim::StatsRegistry::HIGH_PRIORITY do |components, start_at, end_at|
    Decidim::Questions::FilteredQuestions.for(components, start_at, end_at).accepted.not_hidden.count
  end
  ...
  component.register_stat :endorsements_count, priority: Decidim::StatsRegistry::MEDIUM_PRIORITY do |components, start_at, end_at|
    ...
    Decidim::Endorsement.where(resource_id: questions.pluck(:id), resource_type: Decidim::Questions::Question.name).count
  end
  ...
  component.register_stat :followers_count, tag: :followers, priority: Decidim::StatsRegistry::LOW_PRIORITY do |components, start_at, end_at|
    questions_ids = Decidim::Questions::FilteredQuestions.for(components, start_at, end_at).published.not_hidden.pluck(:id)
    Decidim::Follow.where(decidim_followable_type: "Decidim::Questions::Question", decidim_followable_id: questions_ids).count
  end
  ...
  component.exports :questions do |exports|
    exports.collection do |component_instance, user|
      ...
      collection = Decidim::Questions::Question
                   .published
  ...
  component.seeds do |participatory_space|
    ...
    5.times do |n|
      ...
      (n % 3).times do |m|
        ...
        Decidim::Questions::QuestionVote.create!(question: question, author: author) unless question.published_state? && question.rejected?
        Decidim::Questions::QuestionVote.create!(question: emendation, author: author) if emendation
      end

      unless question.published_state? && question.rejected?
        (n * 2).times do |index|
          ...
          Decidim::Endorsement.create!(resource: question, author: author, user_group: author.user_groups.first)
        end
      end
