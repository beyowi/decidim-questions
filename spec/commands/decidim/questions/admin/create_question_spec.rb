          ...
          describe "when the form is valid" do
            ...
            context "when question comes from a meeting" do
              ...
              it "sets the meeting as author" do
                ...
                expect(Decidim::Questions::Question.last.authors).to include(meeting_as_author)
              end

              it "links the question and the meeting" do
                command.call
                question = Decidim::Questions::Question.last
                question_linked_meetings = question.linked_resources(:meeting, "questions_from_meeting")

                expect(question_linked_meetings).to include(meeting_as_author)
              end

              context "when the meeting is already linked to other questions" do
                let(:another_question) { create :question, component: component }

                it "keeps the old questions linked" do
                  another_question.link_resources(meeting_as_author, "questions_from_meeting")
                  command.call
                  question = Decidim::Questions::Question.last
                  linked_questions = meeting_as_author.linked_resources(:question, "questions_from_meeting")

                  expect(linked_questions).to match_array([question, another_question])
                end
              end
            end
            ...
            context "when galleries are allowed" do
              it_behaves_like "admin creates resource gallery" do
                ...
                let(:resource_class) { Decidim::Questions::Question }
