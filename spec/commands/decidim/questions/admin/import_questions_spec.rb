        ...
        describe "call" do
          ...
          describe "when the form is valid" do
            ...
            describe "when keep_authors is true" do
              ...
              it "only keeps the question authors" do
                command.call

                new_question = Question.where(component: current_component).last
                expect(new_question.title).to eq(question.title)
                expect(new_question.body).to eq(question.body)
                expect(new_question.creator_author).to eq(question.creator_author)
              end
            end
            ...
            describe "when the question has attachments" do
              let!(:attachment) do
                create(:attachment, attached_to: question)
              end
              ...
              it "duplicates the attachments" do
                ...
                new_question = Question.where(component: current_component).last
                expect(new_question.attachments.count).to eq(1)
              end
            end
          end
        end
