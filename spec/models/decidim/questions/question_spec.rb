  ...
  module Questions
    describe Question do
      ...
      describe "newsletter participants" do
        subject { Decidim::Questions::Question.newsletter_participant_ids(question.component) }

        let!(:component_out_of_newsletter) { create(:question_component, organization: organization) }
        let!(:resource_out_of_newsletter) { create(:question, component: component_out_of_newsletter) }
        let!(:resource_in_newsletter) { create(:question, component: question.component) }
        let(:author_ids) { question.notifiable_identities.pluck(:id) + resource_in_newsletter.notifiable_identities.pluck(:id) }
      ...
      context "when answer is not published" do
        ...
        it "has accepted as the internal state" do
          expect(question.internal_state).to eq("accepted")
        end

        it "has not_answered as public state" do
          expect(question.state).to be_nil
        end
      ...
      describe "#with_valuation_assigned_to" do
        ...
        let(:assigned_question) { create :question, component: component }
        let!(:assignment) { create :valuation_assignment, question: assigned_question, valuator_role: valuator_role }

        it "only returns the assigned questions for the given space" do
          results = described_class.with_valuation_assigned_to(user, space)

          expect(results).to eq([assigned_question])
        end
      end
    end
  end
