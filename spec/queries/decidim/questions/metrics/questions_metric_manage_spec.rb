...
describe Decidim::Questions::Metrics::QuestionsMetricManage do
  ...
  let!(:questions) { create_list(:question, 3, published_at: day, component: component) }
  ...
  context "when executing" do
    ...
    context "when calculating the metrics" do
      let(:moderation) { create(:moderation, reportable: questions[0], report_count: 1, participatory_space: participatory_space) }
      let!(:report) { create(:report, moderation: moderation) }

      it "filters the data correctly" do
        questions[0].moderation.update!(hidden_at: Time.current)
        questions[1].update!(published_at: nil)
        questions[2].update!(state: "withdrawn")
