...
describe Decidim::Questions::Metrics::VotesMetricManage do
  ...
  context "when executing" do
    ...
    context "when calculating the metrics" do
      let(:withdrawn_question) { create(:question, state: "withdrawn", component: component) }
      let!(:invalid_votes) { create_list(:question_vote, 5, question: withdrawn_question, created_at: day) }
      let(:moderation) { create(:moderation, reportable: question, report_count: 1, participatory_space: participatory_space) }
      let!(:report) { create(:report, moderation: moderation) }

      it "filters the data correctly" do
        question.moderation.update!(hidden_at: Time.current)
