...
describe "Explore Collaborative Drafts", versioning: true, type: :system do
  ...
  let(:request_access_form) { Decidim::Questions::RequestAccessToCollaborativeDraftForm.from_params(state: collaborative_draft.state, id: collaborative_draft.id) }
  let!(:other_user) { create :user, :confirmed, organization: organization }
  let(:request_access_from_other_user) { Decidim::Questions::RequestAccessToCollaborativeDraft.new(request_access_form, other_user) }
