...
describe "Filter Questions", :slow, type: :system do
  ...
  context "when filtering questions by ORIGIN" do
  ...
  context "when filtering questions by SCOPE" do
  ...
  context "when filtering questions by STATE" do
    context "when question_answering component setting is enabled" do
      ...
      context "when question_answering step setting is enabled" do
        ...
        context "when there are questions with answers not published" do
          let!(:question) { create(:question, :accepted_not_published, component: component, scope: scope) }
          ...
          it "shows only accepted questions with published answers" do
            within ".filters .state_check_boxes_tree_filter" do
              check "All"
              uncheck "All"
              check "Accepted"
          ...
          it "shows accepted questions with not published answers as not answered" do
            within ".filters .state_check_boxes_tree_filter" do
              check "All"
              uncheck "All"
              check "Not answered"
  ...
  context "when filtering questions by CATEGORY", :slow do
    context "when the user is logged in" do
      ...
      let!(:question1) { create(:question, component: component, category: category) }
      let!(:question2) { create(:question, component: component, category: category2) }
      let!(:question3) { create(:question, component: component, category: category3) }
  ...
  context "when filtering questions by ACTIVITY" do
    let(:active_step_id) { component.participatory_space.active_step.id }
    let!(:voted_question) { create(:question, component: component) }
    let!(:vote) { create(:question_vote, question: voted_question, author: user) }
    let!(:question_list) { create_list(:question, 3, component: component) }
    let!(:created_question) { create(:question, component: component, users: [user]) }
    ...
    context "when the user is logged in" do
      ...
      it "can be filtered by my questions" do
        within "form.new_filter" do
          expect(page).to have_content(/My questions/i)
        end
      end

      it "lists the filtered questions created by the user" do
        within "form.new_filter" do
          find("input[value='my_questions']").click
        end
        expect(page).to have_css(".card--question", count: 1)
      end
      ...
      context "when votes are enabled" do
        ...
        it "lists the filtered questions voted by the user" do
          within "form.new_filter" do
            find("input[value='voted']").click
          end

          expect(page).to have_css(".card--question", text: voted_question.title)
        end
      end
  ...
  context "when filtering questions by TYPE" do
    context "when there are amendments to questions" do
      ...
      context "when amendments_enabled component setting is enabled" do
        ...
        context "and amendments_visbility component step_setting is set to 'participants'" do
          ...
          context "when the user is logged in" do
            context "and has amended a question" do
              let!(:new_emendation) { create(:question, component: component, scope: scope) }
              let!(:new_amendment) { create(:amendment, amendable: question, emendation: new_emendation, amender: new_emendation.creator_author) }
            ...
            context "and has NOT amended a question" do
              before do
                login_as user, scope: :user
                visit_component
              end
          ...
          context "when the user is logged in" do
            context "and has amended a question" do
              ...
              it "lists all the amendments" do
                ...
                expect(page).to have_css(".card.card--question", count: 2)
                expect(page).to have_content("2 QUESTION")
  ...
  context "when using the browser history", :slow do
    before do
      ...
      create_list(:question, 2, :official, component: component)
      create_list(:question, 2, :official, :accepted, component: component)
      create_list(:question, 2, :official, :rejected, component: component)
    ...
    it "recover filters from initial pages" do
      ...
      expect(page).to have_css(".card.card--question", count: 8)

      page.go_back

      expect(page).to have_css(".card.card--question", count: 6)
    end
