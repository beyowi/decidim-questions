...
describe "Admin manages participatory texts", type: :system do
  ...
  def import_document
    ...
    expect(page).to have_content "The following sections have been converted to questions. Now you can review and adjust them before publishing."
    expect(page).to have_content "Preview participatory text"
  end
  ...
  def edit_participatory_text_body(index, new_body)
    fill_in(
      "preview_participatory_text_questions_attributes_#{index}_body",
      with: new_body
    )
  end
