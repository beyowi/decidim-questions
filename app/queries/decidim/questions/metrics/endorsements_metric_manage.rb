    ...
    module Metrics
      class EndorsementsMetricManage < Decidim::MetricManage
        ...
        def query
          ...
          join_components = "INNER JOIN decidim_components ON decidim_components.manifest_name = 'questions' AND questions.decidim_component_id = decidim_components.id"
          join_categories = <<~EOJOINCATS
            LEFT OUTER JOIN decidim_categorizations
            ON (questions.id = decidim_categorizations.categorizable_id
            AND decidim_categorizations.categorizable_type = 'Decidim::Questions::Question')
          EOJOINCATS
          @query = Decidim::Endorsement.joins("INNER JOIN decidim_questions_questions questions ON resource_id = questions.id")
                                       .joins(join_components)
                                       .joins(join_categories)
                                       .where(resource_id: questions.pluck(:id))
                                       .where(resource_type: Decidim::Questions::Question.name)
