  ...
  module Questions
    class DiffRenderer < BaseDiffRenderer
      ...
      def parse_values(attribute, values)
        values = [amended_previous_value(attribute), values[1]] if question&.emendation?
        values = values.map { |value| normalize_line_endings(value) } if attribute == :body
        values
      end

      # Sets the previous value so the emendation can be compared with the amended question.
      # If the amendment is being evaluated, returns the CURRENT attribute value of the amended question;
      # else, returns the attribute value of amended question at the moment of making the amendment.
      def amended_previous_value(attribute)
        if question.amendment.evaluating?
          question.amendable.attributes[attribute.to_s]
        else # See Decidim::Amendable::PublishDraft#set_first_emendation_version
          question.versions.first.changeset[attribute.to_s].last
        end
      end
      ...
      def question
        @question ||= Question.find_by(id: version.item_id)
      end
    end
  end
end
