  ...
  module Questions
    ...
    class QuestionSearch < ResourceSearch
      ...
      def initialize(options = {})
        ...
        base = options[:state]&.member?("withdrawn") ? Question.withdrawn : Question.except_withdrawn
        super(base, options)
      end
      ...
      def search_search_text
        query
          .where("decidim_questions_questions.title ILIKE ?", "%#{search_text}%")
          .or(query.where("decidim_questions_questions.body ILIKE ?", "%#{search_text}%"))
      end
      ...
      def search_activity
        ...
        when "voted"
          query
            .includes(:votes)
            .where(decidim_questions_question_votes: { decidim_author_id: @current_user })
        when "my_questions"
          query
