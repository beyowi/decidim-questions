  ...
  module Questions
    ...
    module ParticipatoryTextsHelper
      ...
      def preview_participatory_text_section_title(question)
        title = decidim_html_escape(present(question).title)
        translated = t(question.participatory_text_level, scope: "decidim.questions.admin.participatory_texts.sections", title: title)
      ...
      def mime_types_with_document_examples
        links = ""
        accepted_mime_types = Decidim::Questions::DocToMarkdown::ACCEPTED_MIME_TYPES.keys
        accepted_mime_types.each_with_index do |mime_type, index|
          links += link_to t(".accepted_mime_types.#{mime_type}"),
                           asset_path("decidim/questions/participatory_texts/participatory_text.#{mime_type}"),
