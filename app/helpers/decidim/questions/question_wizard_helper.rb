  ...
  module Questions
    ...
    module QuestionWizardHelper
      ...
      def question_wizard_stepper_step(step, current_step)
        ...
        attributes = { class: question_wizard_step_classes(step, current_step).to_s }
        step_title = question_wizard_step_name(step)
        if step.to_s.split("_").last.to_i == question_wizard_step_number(current_step)
          current_step_title = question_wizard_step_name("current_step")
      ...
      def question_wizard_steps_title
        t("title", scope: "decidim.questions.#{type_of}.wizard_steps")
      end
      ...
      def question_wizard_aside_link_to_back(step)
        url = case step
              when :step_1
                questions_path
              when :step_3
                compare_question_path
              when :step_4
                edit_draft_question_path
              end
        url
      end
      ...
      def wizard_aside_back_text(from = nil)
        ...
        case type_of
        when :collaborative_drafts
          t(key, scope: "decidim.questions.collaborative_drafts.wizard_aside").html_safe
        else
          t(key, scope: "decidim.questions.questions.wizard_aside").html_safe
        end
      end
