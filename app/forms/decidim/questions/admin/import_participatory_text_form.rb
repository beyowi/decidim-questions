  ...
  module Questions
    module Admin
      ...
      class ImportParticipatoryTextForm < Decidim::Form
        ...
        ACCEPTED_MIME_TYPES = Decidim::Questions::DocToMarkdown::ACCEPTED_MIME_TYPES
        ...
        # Assume it's a NEW participatory_text if there are no questions
        # Validate document presence while CREATING questions from document
        # Allow skipping document validation while UPDATING title/description
        def new_participatory_text?
          Decidim::Questions::Question.where(component: current_component).blank?
        end
        ...
        def valid_mime_types
          ACCEPTED_MIME_TYPES.values + [Decidim::Questions::DocToMarkdown::TEXT_PLAIN_MIME_TYPE]
        end
        ...
        def i18n_valid_mime_types_text
          ACCEPTED_MIME_TYPES.keys.map do |mime_type|
            I18n.t(mime_type, scope: "decidim.questions.admin.participatory_texts.new_import.accepted_mime_types")
          end.join(", ")
        end
